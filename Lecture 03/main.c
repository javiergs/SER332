...
// myInit
void myInit() {
  glClearColor(0.764f, 0.129f, 0.282f, 1.0); // color maroon
  glColor3f(1.0f, 0.843f, 0.0f);  			// color gold
...
void myDisplay() {
  ...
  glBegin(GL_POLYGON);  						// fill connected polygon
  	  glVertex2f(-0.7, 0.7);  				// vertices of the triangle 1
  	  glVertex2f(0.6, 0.7);
  	  glVertex2f(-0.7, -0.6);
  glEnd();
  glBegin(GL_POLYGON);  						// fill connected polygon
  	  ...
  	  glVertex2f(0.7, -0.7);
  glEnd();
  glFlush();  								//forces commands to execute
}
